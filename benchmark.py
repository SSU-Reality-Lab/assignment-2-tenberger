...
if __name__ == "__main__":
    import os, cv2, numpy as np, features

    BASE = os.path.dirname(os.path.abspath(__file__))
    DIR  = os.path.join(BASE, "resources")  # yosemite1/2.jpg와 H1to2p가 있는 폴더

    keypointDetector = features.HarrisKeypointDetector()
    featureDescriptor = features.MOPSFeatureDescriptor()   # 또는 SimpleFeatureDescriptor()
    featureMatcher    = features.SSDFeatureMatcher()       # 또는 RatioFeatureMatcher()

    kpThreshold    = 0.01   # 코너 점수 하한 (필요시 조정)
    matchThreshold = 3.0    # GT와의 허용 오차(픽셀) - ROC 라벨링용

    ds, aucs, roc_img = benchmark_dir(
        DIR, keypointDetector, featureDescriptor, featureMatcher,
        kpThreshold, matchThreshold
    )

    print("Per-pair mean pixel errors:", ds)
    print("Per-pair AUCs:", aucs)

    os.makedirs("results", exist_ok=True)
    cv2.imwrite(os.path.join("results", "roc_curve.png"), roc_img)
